...
ggplot(mapping = aes(x = as.Date(high_avgs$date)[which(as.Date(high_avgs$date) > '2019-01-01')])) +
  ...
  labs(title = 'Seasonally and First-Differenced Predictions', subtitle = 'MSE: 368.54', x = 'Date', y = 'Price [$]')
ggplot(mapping = aes(x = as.Date(high_avgs$date)[which(as.Date(high_avgs$date) > '2019-01-01')])) +
  geom_path(aes(y = high_avgs$mean_tot[which(as.Date(high_avgs$date) > '2019-01-01')])) +
  geom_path(aes(y = c(high_ts$high_tot_ts[which(as.Date(high_ts$date) > '2019-01-01')], cumsumSeries2)), color = 'red') +
  labs(title = 'First-Differenced and Seasonally Modelled Predictions', subtitle = 'MSE: 1084.40', x = 'Date', y = 'Price [$]')
ggplot(mapping = aes(x = as.Date(high_avgs$date)[which(as.Date(high_avgs$date) > '2019-01-01')])) +
  geom_path(aes(y = high_avgs$mean_tot[which(as.Date(high_avgs$date) > '2019-01-01')])) +
